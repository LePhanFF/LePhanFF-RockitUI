...
const App: React.FC = () => {
  ...
  // Initialize: Fetch file list from GitHub for the local-analysis-format folder
  useEffect(() => {
  ...
  const handleFileSelect = async (fileName: string) => {
    ...
    try {
      const res = await fetch(`${RAW_URL}/${GITHUB_REPO}/main/${LOCAL_PATH}/${fileName}`);
  ...
  const processedSnapshots = useMemo(() => {
    return (snapshots || []).map(s => {
      ...
      try {
        // Find raw text string containing analysis
        ...
        console.warn("LLM Output Normalization Error:", e);
      }

      // Reconstruct consistent input structure if fields are flattened
      ...
      <header className="shrink-0 z-50 bg-slate-900/90 backdrop-blur-xl border-b border-slate-800 px-6 py-4 flex items-center justify-between shadow-2xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Local Intelligence Terminal v5.6</p>
          </div>
        </div>
        ...
        {currentSnapshot && (
          <div className="flex items-center gap-8">
            ...
            <div className={`px-5 py-2.5 rounded-xl flex items-center gap-3 border transition-all shadow-xl ${
              ...
              <span className="font-black text-sm tracking-widest uppercase italic">{decodedOutput?.bias || 'NEUTRAL'}</span>
            </div>
          </div>
        )}
      </header>
